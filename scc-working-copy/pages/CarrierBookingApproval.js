class CarrierBookingApprovalPage {
    ...
    async selectAndApproveBooking() {
        ...
        // When multiple rows are present, select only the FIRST row.
        // SCC's Approve button only enables for a single-row selection (same as Edit Booking).
        const targetCheckbox = rowCount > 1
            ? rows.first().locator('input[type="checkbox"]').first()
            : this.frame.locator(this.selectAllCheck);

        await targetCheckbox.waitFor({ state: 'visible', timeout: 15000 });
        await this.waitForGridToBeReady();
        ...
        for (let attempt = 1; attempt <= 8; attempt++) {
            ...
            console.log(`[CarrierBookingApproval] Approve button still disabled on attempt ${attempt}/8 (rows=${rowCount}), retrying...`);
            await new Promise(r => setTimeout(r, 500));
        }
